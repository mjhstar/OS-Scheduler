...
void spn_swap(Process *ps, Process *cur){ // 노드 교환
    Process tmp;
    tmp.pid = ps->pid;
    tmp.done = ps->done;
...
void spn_resort(Scheduler *sc){ // 다음 프로세스를 스케줄러에 등록
    int min = sc->ps->t_service;
    Process *minp = sc->ps;
    Process *cur = sc->ps;
    if(sc->ps->done != 1) return;
    while(cur){
        if(cur->t_arrival <= sc->time){
            ...
            if(min > cur->t_service && cur->done != 1){
                minp = cur;
            }
        }
        cur = cur->next;
...
int SPN_allDone(Scheduler *sc){ // 모두 처리 되었는가?
    Process *cur = sc->ps;
    while(cur){
        if(cur->done == 0)
            return 0;
...
void SPN_Schedule(FILE *f, Scheduler *sc){ // 스케줄링
    Process *ps;
    spn_resort(sc);
    ps = sc->ps;
    if(ps->t_remain > 0){
